...
export default function MarinaMap() {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[#f1f5f9]">
      ...
      {/* --- 우측 상단: 네비게이션 시작/종료 토글 버튼 --- */}
      <div className="absolute top-5 right-5 z-[3000]">
        <button 
          onClick={() => isNavMode ? clearNav() : setIsNavMode(true)}
          className={`flex items-center gap-3 px-6 py-3 rounded-full font-black shadow-2xl transition-all border-2 ${isNavMode ? 'bg-red-500 text-white border-red-400 animate-pulse' : 'bg-white text-[#003366] border-gray-100'}`}
        >
      ...
      {isNavMode && !isRouteDone && (
        <div className="absolute top-24 right-5 z-[2500] w-80 bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl p-6 border border-white animate-in slide-in-from-right">
          ...
          <div className="space-y-3">
            <div onClick={() => setActiveInput('start')} className={`p-4 border-2 rounded-2xl cursor-pointer transition-all ${activeInput === 'start' ? 'border-blue-500 bg-blue-50' : 'border-gray-100 bg-white'}`}>
              <span className="text-[9px] font-black text-blue-500 uppercase">Start</span>
              <p className="font-bold text-sm truncate">{navStart ? navStart.name : '출발 마리나 선택'}</p>
            </div>
            <div onClick={() => setActiveInput('end')} className={`p-4 border-2 rounded-2xl cursor-pointer transition-all ${activeInput === 'end' ? 'border-blue-500 bg-blue-50' : 'border-gray-100 bg-white'}`}>
              <span className="text-[9px] font-black text-red-500 uppercase">Destination</span>
              <p className="font-bold text-sm truncate">{navEnd ? navEnd.name : '도착 마리나 선택'}</p>
            </div>

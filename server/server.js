...
// ============================================================================
// 🧹 [유령선 청소 스케줄러] - 일정 시간 응답이 없는 선박 데이터를 메모리에서 삭제
// ============================================================================

// 10분(600,000 밀리초)마다 이 청소 함수가 백그라운드에서 자동으로 무한 반복 실행되도록 설정합니다.
setInterval(() => {
    // 현재 시간을 가져와서 now 변수에 저장합니다.
    const now = new Date();
    // 이번 턴에 지운 유령선 개수를 셀 변수를 만들고 0으로 초기화합니다.
    let deletedCount = 0;

    // shipCache Map 객체에 저장된 모든 배의 정보를 하나씩 꺼내서 반복문을 돌립니다.
    for (const [mmsi, shipData] of shipCache.entries()) {
        // 이 배가 마지막으로 위치를 보낸 시간을 날짜 객체로 변환합니다.
        const lastUpdated = new Date(shipData.updated_at);
        // 지금 시간과 마지막 업데이트 시간의 차이를 분(minute) 단위로 계산합니다.
        const diffMinutes = (now - lastUpdated) / (1000 * 60);

        // 만약 15분 넘게 위치 업데이트가 없다면 (구역을 벗어났거나 시동이 꺼진 것으로 판단)
        if (diffMinutes > 15) {
            // 메모리(캐시)에서 이 배의 데이터를 완전히 삭제해 버립니다.
    ...
    // 만약 한 척이라도 지운 배가 있다면 터미널에 청소 완료 메시지를 예쁘게 띄웁니다.
    if (deletedCount > 0) {
        // 몇 척을 지웠는지 서버 콘솔에 로그로 남깁니다.
        console.log(`🧹 [캐시 정리] 15분 이상 응답 없는 유령선 ${deletedCount}척 지도에서 삭제 완료`);
    // 조건문 괄호 닫기
    }
// 10분(10 * 60 * 1000) 주기 설정 괄호 닫기
}, 10 * 60 * 1000);

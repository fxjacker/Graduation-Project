...
// 공공데이터포털에서 쓰는 EPSG:5179 좌표계 세팅
proj4.defs("EPSG:5179", "+proj=tmerc +lat_0=38 +lon_0=127.5 +k=0.9996 +x_0=1000000 +y_0=2000000 +ellps=GRS80 +units=m +no_defs");

// 위도/경도를 기상청 nx, ny 격자로 변환해주는 함수 
function getKmaGrid(lat, lon) {
    // 기상청 공식 문서에 있는 지구 반지름이랑 기준점 상수 
    const RE = 6371.00877;
    const GRID = 5.0;
    const SLAT1 = 30.0;
    const SLAT2 = 60.0;
...
const results = [];
console.log('🚀 마리나 데이터 변환 앤드 DB 적재 스크립트');

// 다운받은 CSV 파일 읽기
fs.createReadStream('해양수산부_마리나 정보_20250124.csv')
    .pipe(csv())
    .on('data', (data) => {
        // 정규식 써서 괄호 안에 있는 숫자만 추출
    ...
    .on('end', async () => {
        // 배열에 담긴 39개 데이터를 Supabase에 한방에 정리
